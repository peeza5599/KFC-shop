...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'C:\\SlipPayment'); // ชื่อโฟลเดอร์ที่คุณต้องการเก็บไฟล์
  },
...
var payAddress = req.body.PayAddress;
var payTel = req.body.PayTel;
var cmID = req.body.cmID;
var TotalAll = req.body.TotalAll;
var slipFilename = req.file.filename;
...
var values =  {
  ...
  TotalAll: TotalAll,
  PayTel: payTel,
  SlipPayment: slipFilename, // ใช้ชื่อไฟล์ที่ถูกอัพโหลด
    ...
    if (err) {
      ...
      mysql.query(sql, (err, paymentResult) => {
        if (err) {
          ...
          var sql3 = 'SELECT products.productName as productName, orders.Quantity as Quantity, orders.SubTotalPrice as SubTotalPrice FROM products JOIN orders ON products.productID = orders.ProductID WHERE orders.cmID = ? ORDER BY orderDate DESC LIMIT 3';
